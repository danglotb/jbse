...
public final class Run {
    ...
    private void createFormatter() throws CannotBuildFormatterException {
        ...
        if (type == StateFormatMode.FULLTEXT) {
        ...
        } else if (type == StateFormatMode.FULLTEXTHISTORY) {
            this.formatterBranches = new StateFormatterTextWithHistory(this.parameters.getSourcePath()) {
                @Override
                public void formatState(State s) {
                    this.output += LINE_SEP; // gutter
                    this.output +=
            ...
            this.formatterOthers = new StateFormatterTextWithHistory(this.parameters.getSourcePath()) {
                @Override
                public void formatState(State s) {
                    this.output += LINE_SEP; // gutter
                    this.output +=
    ...
    private void createDecisionProcedure(CalculatorRewriting calc)
        ...
        if (this.parameters.getUseConservativeRepOks()) {
            ...
            @SuppressWarnings("resource") final DecisionProcedureConservativeRepOk dec =
